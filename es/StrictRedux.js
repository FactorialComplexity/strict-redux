...
var _initialiseProps = function _initialiseProps() {
  ...
  this.getActionTypes = function (queryString) {
    if (queryString) {
      return _this._normalizeQuery(queryString, '_actionCreators').reduce(function (acc, name) {
        var _extends6;
    ...
    return Object.keys(_this._actionCreators).reduce(function (acc, name) {
      ...
      return _extends({}, acc, (_extends7 = {}, _extends7[name] = name, _extends7));
    }, {});
  };
  ...
  this._buildStore = function (slices, middlewareFactories) {
    ...
    var middleware = applyMiddleware.apply(undefined, middlewareFactories.map(function (factory) {
      return factory(_this);
    }));

    // Assign store to pre-created empty object to make it available in middleware factories
    Object.assign(_this._store, createStore(combineReducers(_this._reducers), devTools ? compose(middleware, devTools) : middleware));
  };
  ...
  this._createReducer = function (sliceDescriptor) {
    return function () {
      ...
      if (action.type.startsWith(sliceDescriptor.sliceName) && sliceDescriptor.actionReducers[sliceActionType]) {
        return _extends({}, state, sliceDescriptor.actionReducers[sliceActionType](state, action.payload));
      } else {
        return state;
      }
    };
  ...
  this._createSelectors = function (sliceDescriptor) {
    ...
    if (typeof sliceDescriptor.createSelectors === 'function') {
      allSelectors = _extends({}, defaultSelectors, sliceDescriptor.createSelectors(_this.select, _this.selectOne));
    } else {
